...
9b. **"What impression does it give?" / Ghost-reader perspective (MANDATORY).** Re-read every bullet as a **ghost reader** — the skeptical hiring manager skimming it cold, fast, with no context and no goodwill. In one sentence, state the 5-second takeaway: what did this person *do* and at what level? Then stress-test it as that reader would: *Does it land in 5 seconds, or do I have to parse it? What's the one number my eye catches? Would I believe this claim, or does it smell inflated? Do I know what they actually did, or just that words happened?* If the impression is "did tasks" rather than "drove an outcome," or the eye snags on jargon before hitting a result, the bullet is mis-weighted — fix before compiling. The ghost reader is unimpressed by default; the bullet has to earn the read.
...
9g. **Verb-first, minimize "I" (résumé convention — MANDATORY).** Every bullet's **first word should be the action verb** — the strongest, most scannable opener. This means:
   - **No leading subject:** "Architected the harness," not "I architected the harness." Repeated "I built… I authored… I extended…" reads as a personal essay and crowds the verb out of the 5-second scan. Cut leading "I"s; keep at most one deliberate first-person where solo ownership is the point ("Personally built…" to stress a one-person effort).
   - **No leading framing phrase:** don't open with a prepositional or infinitive setup that delays the verb — "To prove it scales, architected…" or "In that window, built…" bury the action. Lead with the verb and move the *why* to the end: "Architected a stress-testing harness… to ensure the infra handles production load."
   - **Past tense** (see also the past-tense convention): "Architected," "Built," "Drove" — not "architecting," "am developing." Convert "I am developing" → "developed" or "actively developed."

9h. **Sentence boundaries & cadence (MANDATORY for multi-sentence bullets).** A multi-sentence bullet is only as good as where it breaks. Two failures make a bullet read as AI-generated even when every fact is true:
   - **Arbitrary breakpoints — break at *idea* seams, not mid-thought.** A mid-bullet period asks the reader to *stop, reset, and start a new thought*, so every period must fall at a genuine idea boundary. The tell-tale anti-pattern is the **triple-stuffed sentence**: three unrelated items fused with "comma… and… and" ("Ran each agent in a container, sized throughput at the Azure layer, and built a stress-testing harness"). That isn't one idea — it's three, and the "and"-chain is itself an LLM cadence. Fix: **one idea per sentence**, split at the seam (the runtime / the proof / the safety work / the tuning win each get their own sentence). Test by asking of each sentence: *is this one thought, or did I staple three together?*
   - **Uniform cadence — vary sentence length deliberately (burstiness).** AI prose drones at a metronomic 15–25 words per sentence; human writing is *bursty* — a short punch, then a longer sentence carrying nuance, then something simple. After splitting at idea seams, read the bullet aloud: if every sentence is the same length, collapse one to a short declarative ("Parallelism came with risks.") or let one run longer. Stumbling, pausing, or thinking "I'd never say this" while reading aloud marks the exact spot to rewrite.
   - **Connective tissue over fused lists.** Replace the fused colon-list and the comma-and-comma-and triad with light human connectors that name the *relationship* between sentences ("Parallelism came with risks:", "Separately,", "That work shaped…"). These signal a person organizing thoughts, not a model emitting a list.
   - This sub-check is the natural-language companion to 9f (which catches the tells) and 9g (which fixes openers): 9f says *remove em-dashes and triads*, 9g says *lead with the verb*, **9h says *break and pace the sentences like a person would*.**
