...
| B-15 | `classifyRow` (lost) + `classify_row` (Python audit) | **Redirect-stub FP**: when a destination plan has been migrated (`> Migrated: see [[NewPlan]]`), lines classified as lost against the stub now follow the redirect and recheck against the linked plan's diff additions. JS: checks DEST_FILE_LINES (disk) and DIFF_TEXT context lines. Python audit: reads dest file on disk for redirect marker. JS-18 regression test. | 3.100.8 / 3.102.1 |
