...
| **Airbnb rental listing** | `airbnb.com/rooms/<id>` — room ID is a plain integer (not a UUID, so not auto-skipped) | **Use page title** — format is reliable: "Property Name - Type for Rent in Location, Country - Airbnb"; strip all tracking params (`irclid`, `afsrc`, `sharedid`, `irgwc`, `source_impression_id`, etc.) | `[Airbnb: The Upstate A — modern luxury cabin in Pine Plains, Hudson Valley NY](url)` |
| **MasterClass course/certificate** | `masterclass.com/certificates/<slug>`, `masterclass.com/courses/<slug>` | **Label from URL slug** — HTTP 403 (blocks bots); replace dashes with spaces, capitalize words, insert em dash before "from/to/a/an" where logical | `[MasterClass: Breakthrough Innovation Strategy — from bold idea to business impact](url)` |
| **HackerOne program** | `hackerone.com/<program>/...` | **Label from program name + path context** — HTTP 403 (requires login); decode `<program>` slug and use path to describe action (e.g. `reports/new` → "submit report") | `[HackerOne: submit vulnerability report to Anthropic VDP](url)` |
| **GitHub Wiki** | `github.com/<owner>/<repo>/wiki` or `.../wiki/<Page>` | **Use description for homepage, title for specific pages** — homepage title "Home · repo Wiki · GitHub" is weak; description has the repo purpose; specific wiki pages have descriptive titles | `[tsightler/ring-mqtt wiki: Ring devices to MQTT bridge documentation](url)` |
